...
| 테스트 횟수 | 결과 일치 | 평균 전투 시간 | 평균 재현 시간 | 평균 프레임 수 |
| ---: | ---: | ---: | ---: | ---: |
| 105회 | 105/105 | 319.5초 | 129.7ms | 19,306.7 |

충분한 전투 길이를 확보하기 위해 동일한 맵에서 모든 Entity의 `maxHp`를 29배로 높여 측정했습니다. 재현 시간에는 서버 시뮬레이터 생성과 초기화, 기록된 프레임 간격 재현, 최종 결과 비교가 포함되며 HTTP 통신, DB 조회와 맵 데이터 로딩은 포함되지 않습니다.
